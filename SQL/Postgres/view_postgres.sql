...
SELECT
    ...
    j.data_nascimento,
    j.genero,
    STRING_AGG(DISTINCT t.name, ', ') AS time_atual,
...
GROUP BY j.id, j.name, j.apelido, j.nacionalidade, j.data_nascimento, j.genero, e.nome, e.id, trof.foto_id, f.nome_foto, f.id;
